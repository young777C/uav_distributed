...
> **优先级**:H0 是门(不过则全盘停);H4 已近完成;H1/H2/H3 是方法贡献主体;H5 是外部可比性;H6 后置可选。**H7(路线 B/SEP)后置于 H1/H2 之后,且依赖长丢失数据 → 与 scale-up 同批,MVP 仅做方向性 + guard 验证**。
...
> 目标:证明 SEP(时不变路网先验)在**长丢失预测拦截**(design §6.1 招牌场景)上抬升 EAR,**且不放大位置捷径**(§2.0c guard)。SEP 详义与先例切割见 related-work §4D。

- **对照**:M0(EAR-only) vs M11(EAR+SEP);同 held-out、3 seed、配对(§8)。加 **M11g**(SEP 喂语言)作 guard 反向对照。
