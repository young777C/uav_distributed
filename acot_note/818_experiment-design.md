...
| **H7(路线B/SEP)** | 结构化环境先验(SEP,时不变路网)提升长丢失预测拦截,**且不放大位置捷径** | 去 SEP→长丢失重捕获变差;加 SEP 后 w/o-lang 增幅**不缩小**(g2) | EAR+SEP vs EAR-only;并在开启 SEP 下重跑 E1/E5 | 长丢失 minFDE、重捕获锁对率/收敛时间、**w/o-lang 增幅不降** | 离线 + 闭环 |
| **H8(语言 which-way)** | 语言除消歧(which)外,作**意图种子**塑造预测(which-way),且携带 SEP 路网+运动**推不出**的增量信息 | 意图语言只在"分叉歧义"场景有净贡献;若能被路网+运动推出则冗余 | 三臂消融(无语言/仅消歧/消歧+意图)+ 分叉歧义场景 + 意图冗余 guard | 分叉场景 minFDE↓、正确分支率、意图语言净贡献(vs 仅消歧臂) | 离线 + 闭环 |

> **优先级**:H0 是门(不过则全盘停);H4 已近完成;H1/H2/H3 是方法贡献主体;H5 是外部可比性;H6 后置可选。**H7(路线 B/SEP)后置于 H1/H2 之后,且依赖长丢失数据 → 与 scale-up 同批,MVP 仅做方向性 + guard 验证**。**H8(语言 which-way 意图种子)是叠在 SEP 之上的第二条轴,依赖富意图语言 + 分叉歧义场景数据 → 最后置,MVP 不做,仅在 §2.0d 立设计与 guard**。
...
### 2.0c 结构化环境先验(路线 B/SEP)对 H0 的影响:2×2 分工 + 非放大 guard(2026-08-06)

> 路线 B 加入 **SEP**(从航拍视频在线推**时不变路网结构**,作 EAR 先验;详义与先例切割见 related-work §4D)。**关键风险**:SEP 预测"路上车往哪走"是**身份无关**的——单主干道场景里"跟着路走"几乎必对 → 可能**放大位置捷径、进一步掏空 H0**(memory:position-only 已 ~3–5% mis_follow)。

**用 2×2 把风险转成设计特性**(时间平稳性 × 身份性):

|  | 身份无关 | 身份相关 |
|---|---|---|
| **时不变** | 路网/车道/可行驶区(**SEP**) | ∅ |
| **时变** | 流密流速场、遮挡、全体占据 | **目标 vs 干扰选择 → 语言独占** |

SEP 只占左列(身份无关)→ **结构上不可能解 which**:左上分不开同路 look-alike、左下流态只说"有车往这走"非"你的车"、右上为空 → which 的唯一归属是右下,重现时外观又失效 → **语言是唯一出口**。故 SEP 越准,where 越被它解释干净,which 越只剩语言(反而**加强** H0 论证)。

**非放大 guard(写死判据,防 SEP 偷 H0 的功劳)**:
```
加 SEP 后必须同时满足(否则 SEP 违规 → 退回 EAR-only 或改造):
 (g1) 长丢失重捕获锁对率 / 长丢失 minFDE 显著优于 EAR-only(SEP 有 H1/H7 增益)
 (g2) 加 SEP 后 w/o-language 的 mis_follow 增幅【不缩小】
      (若缩小 → SEP 在替语言消歧 = 泄漏路径,违反 identity-agnostic)
 (g3) SEP 仅吃车道级统计场,不建瞬时逐车占据(架构层堵泄漏)
```
- **实现**:SEP 头**不接收** target-id / 语言 token,仅从视觉推结构;在 E1/E5 的 w/o-language 消融时 **SEP 保持开启** → 直接读出 g2。反向对照见 §5.1 的 **M11g**(故意给 SEP 喂语言→应看到增幅缩小,坐实分工)。

### 2.0d 语言 which-way(意图种子)对 H0 的影响:三臂消融 + 意图冗余 guard(2026-08-11)

> 新方向(related-work §4E)让语言从"只做消歧(which)"扩展为"**同时作意图种子塑造预测(which-way)**"。方法学背书(LC-LLM 双任务、Trajectory-LLM 的 L2T、SWM 预测语义状态)见 `language-seeded-prediction-survey.md`。**关键风险**:语言意图跨进 SEP 的"where 列",既复杂化 H0 归因,又可能落入**意图冗余陷阱**(与位置捷径同构)。

**三臂消融(把两种语言贡献拆开量)**:
```
臂 1  无语言          → 基线(H0 下限)
臂 2  仅消歧语言       → 只指身份("the orange Jeep"),不含意图
臂 3  消歧+意图语言    → 加目的地/行为/意图("...heading to the highway on-ramp")
判据: (臂2 − 臂1)=语言消歧净贡献(which);(臂3 − 臂2)=语言意图净贡献(which-way)
```

**意图冗余 guard(写死,防 which-way 退化为位置捷径)**:
```
which-way 成立 ⟺ (h1) 意图净贡献(臂3−臂2)集中在【分叉歧义】场景
                  ∧ (h2) 该贡献【不能】被 SEP 路网+当前运动推出
                         → 意图须携带"路+运动推不出的信息":
                           视野外目的地 / 未显现行为 / 非最短路径选择
                  ∧ (h3) 反事实意图测试:同帧改意图指令 → 预测分支随之切换
```
- **分叉歧义场景(必造)**:路口/岔路,road+motion 对下一段各 ~50%,**只有语言意图能定向**——这是 which-way 的"杀手时刻",与 §6.3 长丢失消歧的 which 杀手时刻对偶。
- **实现**:意图头**不接收**位置捷径可用的信号;意图语言用 CARLA autopilot 真实路线 GT 自动生成(L2T 式"意图语言↔真值轨迹"配对)。若 (h2) 破(路网+运动已能推出)→ 该场景不计入 which-way 证据。
- **统一 framing 落点**:臂3−臂2 在分叉场景显著 = "语言解决几何+运动定不了的 which-way",与外观分不清时"语言定 which"同一原理 → 加强 H0。
...
**⚠️ 时间容忍度:逐帧 Mis-follow 偏严,须区分"瞬时闪跳"与"持续跟错"**
现有 `_evaluate` 逐帧独立判 `argmax≠target_idx`,**1 帧闪跳(下一帧即恢复)也计错**——但 flow-matching 采样有随机性、两辆相近车会逐帧抖,单帧闪跳 ≠ 跟踪失败。故:
- **逐帧 Mis-follow(instantaneous)**:保留为**瞬时 grounding 诊断**(下限指标),离线报。
- **持续 Mis-follow / ID-switch(sustained)★主指标**:承诺**连续 ≥K 帧**跟错同一辆错车才计一次(建议 K=5 帧 ≈0.5s@10fps);这才等价 MOT 的 ID-switch,反映真实"跟丢/跟错"。
- **迟滞(hysteresis)**:target-id 的"承诺目标"仅当新候选**连赢 K 帧**才切换 → 既是系统鲁棒性设计,也让评测只惩罚真正的持续切换。逐帧 argmax 仍记录(诊断),但 SR/ID-switch 基于迟滞后的承诺轨迹。
- 报告时**并列**逐帧(严)与持续(K=5)两个数,避免"逐帧零容忍不合理"的质疑。
...
| M11 | **ACoT + SEP**(EAR 加时不变路网先验;路线 B 主模型) | 路线B主模型 | H7 | 🟡(需建 SEP 头 + CARLA 路网/车道 GT + 长丢失数据) |
| M11g | **SEP 违规对照**(故意给 SEP 喂语言/target-id) | guard 对照 | H7/H0(验 g2 泄漏) | 🟡(仅需改 SEP 输入,复用 M11 脚手架) |
| M12 | **ACoT + 意图头**(语言 which-way:LC-LLM 式意图+轨迹双任务) | 路线C主模型 | H8 | 🟡(需建意图头 + 富意图语言 + 分叉歧义场景 + L2T 式 GT 配对) |
| M12-三臂 | **无语言 / 仅消歧语言 / 消歧+意图语言** | 关键对照 | H8/H0(拆 which vs which-way,§2.0d) | 🟡(复用 M12 脚手架,换语言输入档) |
...
> **⚑ 双重角色 = 评测 + Stage-3 训练底座(2026-08-14,与 `training-plan.md` Phase E.5 互引)**
> 本节 harness 不只是【评测】工具,更是 **Stage-3 闭环 RL 精调的训练底座**,是其**硬前置依赖**:
> - **原理**:RL 的目标 = "策略在环境里飞出的轨迹分布"上的期望。没有环境在训练回路里,就**没有轨迹、没有奖励、没有可优化对象**——on-policy 数据须由**当前策略现场 rollout 生成**、状态分布由**策略自身动作**决定(闭环 RL 的全部意义就是修 BC 的分布漂移)。
> - **对比**:BC(Stage 1-2)**不需要** harness(训练只读固定离线数据集,环境仅一次性造数据);RL(Stage 3)**必须**把环境放进训练回路。
> - **结论**:**先有 harness,才谈得上 Stage-3 的 RL 训练与其闭环 gate**(SR / mis_follow 优于 BC、无 reward hacking)。故 Phase E 排最后、标"可选/后置"。harness 具体设计要求见 `training-plan.md` Phase E.1(在线 VLM 推理)/E.3(特权 GT 算奖励)/E.5(依赖)。
...
**闭环 Mis-follow 的时间判定(与 §3.4 族B 一致,写死口径)**:闭环里 Mis-follow **不逐帧判**,而是判**持续跟错**——
- **承诺目标 + 迟滞**:策略维护一个"承诺目标",新候选须连赢 **K 帧(建议 5 帧≈0.5s)** 才切换;逐帧 argmax 仅记录用于诊断。
- **ID-switch(主指标)**:承诺目标从真目标切到某辆错车并**保持 ≥K 帧** = 一次 mis-follow;瞬时闪跳不计。episode 判"失败"= 承诺错车 ≥T 秒(建议 T=2s)。
- **重捕获场景专属指标**(呼应 §2.1 E5 / data-fix-spec §3):把"允许延迟"与"禁止锁错"分开报 ——
  - **重捕获收敛时间**:丢失结束→稳定锁对目标的帧数(**允许**先扫几帧,不惩罚延迟);
  - **重捕获锁错率**:重现后承诺到错误 look-alike 的比例(**硬失败**,不容忍;这是"语言唯一可分"的核心考验)。

---

### 7.4 结构化环境先验(路线 B/SEP)× 长丢失预测拦截 —— 消融协议(2026-08-06)

> 目标:证明 SEP(时不变路网先验)在**长丢失预测拦截**(design §6.1 招牌场景)上抬升 EAR,**且不放大位置捷径**(§2.0c guard)。SEP 详义与先例切割见 related-work §4D;方法学背书与"显式 3D 状态 vs 纯隐式长上下文"争议见 `world-model-survey.md`(deep-research 2026-08-07)。
> **★ 必加消融(源自 survey 争议)**:M11(SEP,显式路网先验) vs **纯隐式长上下文基线**(把 EAR/IAR 有效上下文按 TTT/WorldPack 式扩容、不给显式路网)—— 因隐式方法(StateSpaceDiffuser/Geometry-Aware Implicit Memory)已达千帧级一致,若此对照下 SEP 增益不显著,则 SEP 卖点须收窄到"长丢失 + 抗漂移可解释性"(见 R10)。抗 drift 训练技巧(KV-cache 自 rollout / error-recycling / BAgger)可迁移进 EAR,均训练侧、不需像素生成。

> **★ no-WM 离线基线已就位(2026-08-18,`train/intercept_eval.py` on `runs/stage2_v5_fair`)**:公平重训(Stage-1a EAR 预热 + 5 层 IAR + `supervise_intercept`)后的 EAR-only 出画预测 = WM/SEP 必须超越的 **M0 对照**。实测 **EAR FDE**:可见 14/21m@1s/6s、**丢失 58/78m@1s/6s**;速度无关 persist 参照丢失 6s=17.6m(**EAR 比"目标不动"还差 4×**);**拦截朝向 cos** 丢失中位 0.87、>0.5 占 66%。关键:公平版 ≈ 欠训 E1 版(丢失 FDE 几乎不变)→ **反应式单帧预测失败是架构性的、训练救不回** → **H7/WM 有据**(详见 `transition-diary-801.md` §八、memory `acot-uav-wm-roadgraph`)。WM 的 minFDE@{6,10}s 与重捕获锁对率须显著优于此基线才算兑现。

- **对照**:M0(EAR-only) vs M11(EAR+SEP);同 held-out、3 seed、配对(§8)。加 **M11g**(SEP 喂语言)作 guard 反向对照。**M0 的离线基线数值见上方 ★ callout**。
- **数据条件(前置)**:须用含**长丢失(5–15s)+ 目标重现 + 拦截式专家演示**的数据(`data-fix-spec.md` §3);无则该实验不可做,标"待 scale"(与 §10 一致)。
- **SEP 侧指标(族 C 扩展)**:
  - **结构预测质量**:预测车道中心线 / 可行驶区 vs CARLA `Waypoint` GT 的偏差(lateral error、拓扑连接 F1);
  - **先验对预测的增益**:长丢失窗口内 **minFDE@{6,10}s** —— SEP 应在**长窗**显著优于运动学 EAR(路约束把 horizon 拉长,是 SEP 的价值区)。
- **拦截/重捕获指标(族 D + §7 重捕获专属)**:
  - **重捕获锁对率**(硬失败口径,§7)、**重捕获收敛时间**(允许延迟);
  - **按丢失时长分层**:短丢失(<6s,EAR horizon 内) vs 长丢失(>6s,SEP 价值区)—— **SEP 增益应集中在长丢失**,否则 SEP 无独立贡献。
- **H0 非放大 guard(§2.0c,写死)**:在**开启 SEP** 下重跑 E1(w/o-language)、E5(断裂后重捕获);判据 **g2 = 加 SEP 后 w/o-lang mis_follow 增幅不缩小**。M11g 应反向显示"违反 identity-agnostic → 增幅缩小",双向坐实"SEP 定 where、语言定 which"分工。
- **单因子消融记账**:SEP 输入 {仅当前帧结构 / +短时序结构累积}、SEP 是否**世界系累积**(自运动补偿开关)→ 验 §4D caveat 3(时不变须世界系累积、非图像系)。
...
| **R9 SEP 放大位置捷径(路线B致命)** | 加 SEP 后 w/o-lang 增幅缩小(g2 破);单主干道"跟着路走"即高 SR | SEP 严格 identity-agnostic(不喂语言/target-id,§2.0c)+ 只吃车道级统计场(g3);破则退回 EAR-only,或把 SEP **降级为纯长丢失先验**(仅在丢失窗激活、不进常规跟踪) |
| **R10 纯隐式长上下文反超 SEP**(survey 争议) | §7.4 消融中 SEP(显式路网) ≈ TTT/WorldPack 式隐式扩容基线 | 隐式方法已达千帧级一致 → 显式结构非唯一路线;预案:SEP 主张收窄到"长丢失 + 抗漂移可解释性",不 claim 通用增益 |
| **R11 EAR 长时漂移不可根治**(survey 空白2) | >10–15s 丢失后拦截点 minFDE 失控 | 抗 drift 机制(自 rollout/error-recycling/BAgger)只缓解;limitation 明写漂移上限存在,长丢失由 search_mode + 语言重捕获兜底 |
| **R12 语言意图冗余 / which-way 退化(路线C致命)** | 三臂中(臂3−臂2)≈0,或意图贡献能被 SEP 路网+运动推出(§2.0d h2 破) | 意图头不接位置捷径信号;贡献须集中在分叉歧义场景且携带路+运动推不出的信息;破则 which-way 主张不成立,退回"语言=消歧(which)"单功能定位、H8 收为 future work |
