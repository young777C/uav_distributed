...
**多指标一致大幅改善 → 真信号(非 SR-latch 偶然)。完整系统(reid+temporal+motion)现为最佳均衡**:追平 assoc_deepsort SR 0.211、正确率更高(0.835>0.750)、错跟更短(2.55<3.20);远优于 DAM4SAM 的 SR/错跟(其粘滞 16s)。**borrowable 排序被证明有预测力**。源 `eval_out/paper_ext_reidmotion.json`(RMOT=0.5)。下一步可扫 RMOT 0.3/0.7 或叠加 ②置信度调制控制。
