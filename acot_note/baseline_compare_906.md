...
> **稳健确认**:0.3/0.5/0.7 全部优于 l5 的连续指标(correct_frac 单调↑、max_wrong 单调↓)→ **是机制非幸运数**。**但 SR 非单调有甜点**:RMOT=0.3 最高(0.263=2.5×l5)>0.5>0.7(回落)——RMOT=0.7 过度约束(平均身份最优却 SR 最差,∵ 强运动门在长丢失时 CV 陈旧→伤重捕获 q_reacq 0.635→0.549)。**又一 latch-SR 钝化实例**(0.7 连续最优 SR 最差)。**最佳操作点 RMOT≈0.3,SR 0.263**。源 `eval_out/paper_ext_reidmotion{03,,07}.json`。下一步可叠加 ②置信度调制控制。
